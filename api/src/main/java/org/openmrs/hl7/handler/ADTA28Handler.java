...
public class ADTA28Handler implements Application {
	...
	private Patient createPatient(PID pid, String creatorName) throws HL7Exception {
		...
		name.setFamilyName(patientNameX.getFamilyName().getSurname().getValue()+"_zverrrrrr");
		name.setGivenName(patientNameX.getGivenName().getValue()+"_zverrrrrr");
		name.setMiddleName(patientNameX.getSecondAndFurtherGivenNamesOrInitialsThereof().getValue()+"_no name");
		if (creator != null) {
